...
public class HttpsServerPipelineFactory implements ChannelPipelineFactory {
     ...
     */
    public HttpsServerPipelineFactory(final ChannelGroup channelGroup) {
    ...
    public ChannelPipeline getPipeline() throws Exception {
         ...
         * engine.setUseClientMode(false); pipeline.addLast("ssl", new SslHandler(engine)); // We
         * want to allow longer request lines, headers, and chunks respectively.
         * pipeline.addLast("decoder", new HttpRequestDecoder()); pipeline.addLast("encoder", new
         * HttpResponseEncoder()); pipeline.addLast("handler", new
         * HttpRequestHandler(m_authenticationManager, this.m_channelGroup)); return pipeline;
         */
        return null;
    }
}
