 ...
 * handles a single connection from the browser. The browser can and will, however, send requests to
 * multiple hosts using that same connection, i.e. it will send a request to host B once a request
...
public class HttpRequestHandler extends SimpleChannelUpstreamHandler implements RelayListener, ConnectionData {
    ...
    /**
     * This lock is necessary for when a second chunk arrives in a request before we've even created
     * the current channel future.
     */
     ...
     * @param authorizationManager The class that handles any proxy authentication requirements.
     * @param channelGroup The group of channels for keeping track of all channels we've opened.
     * @param clientChannelFactory The common channel factory for clients.
     * @param chainProxyManager upstream proxy server host and port or null if none used.
     * @param rewriter
     * @param router
    ...
    private void setupJmx() {
        ...
        try {
            ...
            final String oName = pack + ":type=" + clazz.getSimpleName() + "-" + clazz.getSimpleName() + "-"
                    + hashCode();
    ...
    private void processRequest(final ChannelHandlerContext ctx, final MessageEvent me) {
        ...
        if (config.cacheManager() != null && config.cacheManager().returnCacheHit((HttpRequest) me.getMessage(), inboundChannel)) {
                    ...
                    if (future.isSuccess()) {
                        ...
                        wf.addListener(new ChannelFutureListener() {
                            public void operationComplete(final ChannelFuture wcf) throws Exception {
                                log.info("Finished write: " + wcf + " to: " + request.getMethod() + " "
                                        + request.getUri());
                        ...
                        log.info("Could not connect to " + hostAndPort, future.getCause());

                        final String nextHostAndPort = hostAndPort;
    ...
    private void writeConnectResponse(final ChannelHandlerContext ctx, final HttpRequest httpRequest,
            final Channel outgoingChannel) {
    ...
    public void channelClosed(final ChannelHandlerContext ctx, final ChannelStateEvent cse) {
        ...
        if (browserToProxyConnections.get() == 0) {
            ...
            final Collection<Queue<ChannelFuture>> allFutures = externalHostsToChannelFutures.values();
    ...
    public void onRelayChannelClose(final Channel browserToProxyChannel, final String key,
            final int unansweredRequestsOnChannel, final boolean closedEndsResponseBody) {
        ...
        if (receivedChannelClosed && (externalHostsToChannelFutures.isEmpty() || unansweredRequestCount.get() == 0)) {
            ...
            log.info("Not closing browser to proxy channel. Still " + externalHostsToChannelFutures.size()
                    + " connections and awaiting " + unansweredRequestCount + " responses");
        }
    }
    ...
    public void onRelayHttpResponse(final Channel browserToProxyChannel, final String key, final HttpRequest httpRequest) {
        ...
        if (unansweredRequestCount.get() == 0 && receivedChannelClosed) {
            ...
            log.info("Not closing browser to proxy channel. Still " + "awaiting " + unansweredRequestCount
                    + " responses..." + "receivedChannelClosed=" + receivedChannelClosed);
        }
    }
